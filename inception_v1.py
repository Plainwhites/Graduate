    ...
    with tf.variable_scope(scope, 'InceptionV1', [inputs]):
        with slim.arg_scope(
            ...
            with slim.arg_scope([slim.conv2d, slim.max_pool2d],
                ...
                # the first layer conv-layer kernel 7*7 stride 2 padding 'same'
                end_point = 'Conv2d_1a_7x7'
                net = slim.conv2d(inputs, 64, [7, 7], stride=2, scope=end_point)
                end_points[end_point] = net
                if final_endpoint == end_point:
                    return net, end_points
                # the second layer maxpooling
                end_point = 'MaxPool_2a_3x3'
                net = slim.max_pool2d(net, [3, 3], stride=2, scope=end_point)
                end_points[end_point] = net
                if final_endpoint == end_point:
                    return net, end_points
                # the third layer conv
                # in the paper http://arxiv.org/pdf/1409.4842v1.pdf.
                # there is a LocalRespNorm layer between the second layer and third layer
                # shown in Figure 3
                ...
                if final_endpoint == end_point:
                    return net, end_points
                # the fourth layer conv
                end_point = 'Conv2d_2c_3x3'
                net = slim.conv2d(net, 192, [3, 3], scope=end_point)
                end_points[end_point] = net
                if final_endpoint == end_point:
                    return net, end_points
                # the fifth layer maxpooling
...
#                 logits = slim.conv2d(
#                     net, num_classes, [
#                         1, 1], activation_fn=None, normalizer_fn=None, scope='Conv2d_0c_1x1')
#                 if spatial_squeeze:
